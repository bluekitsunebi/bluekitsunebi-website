...
const Heading = tw(
  SectionHeading
)`mt-4 font-black text-left text-3xl sm:text-4xl lg:text-5xl text-center md:text-left leading-tight text-primary-900`;

const Form = tw.form`mt-8 md:mt-5 text-sm flex flex-col max-w-sm mx-auto md:mx-0`;

const Variants = tw.div`text-gray-600 text-lg mt-10 flex flex-col gap-10 max-w-96 mx-auto md:mx-0`;

const Phone = tw.div`flex gap-2 text-lg justify-center md:justify-start`;

const Divider = tw.div`w-full h-1 bg-gray-300 rounded`;
...
const Checkbox = tw.p`text-base text-gray-600 mt-5 whitespace-normal select-none`;
...
const SubmitButton = tw(
  PrimaryButtonBase
)` mt-8 bg-primary-500 flex justify-center`;
          ...
          <TextContent>
            ...
            <Form onSubmit={sendEmail} action={formAction} method={formMethod}>
              ...
              <Checkbox onClick={() => handleAccord()}>
                ...
                {langData.ContactSection.checkbox.message[0]}
                <Link href="#">
                  {langData.ContactSection.checkbox.message[1]}
                </Link>
                {langData.ContactSection.checkbox.message[2]}
                <Link href="#">
                  {langData.ContactSection.checkbox.message[3]}
                </Link>
                <AccordNeeded>
                  {accordNeeded ? (
                    <Show>
                      ({langData.ContactSection.requiredErrorMessage})
                    </Show>
                  ) : (
                    <Hide>
                      ({langData.ContactSection.requiredErrorMessage})
